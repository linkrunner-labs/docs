...
-   **Click the link before the app opens for the first time.** A click that lands after the install is never matched
    to it.
-   There is no few-minute deadline. The click stays eligible for the campaign's full attribution window — **7 days by
    default**, and 30 days for Google and Meta campaigns.
...
-   Confirm you **clicked the link before opening the app**. A click that lands after the install is never matched to
    it. There is no few-minute deadline — the click stays eligible for the campaign's full attribution window (7 days
    by default, 30 for Google and Meta).
