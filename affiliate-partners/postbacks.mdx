...
| `{is_reattribution}` | `1` if the conversion was credited to a re-engagement, `0` if it is a new acquisition. | `0`     |
...
<Note>
  `{is_reinstall}` and `{is_reattribution}` are not the same signal. A reinstall
  that falls outside the advertiser's re-engagement window arrives as
  `is_reinstall=1` with `is_reattribution=0`.
</Note>
