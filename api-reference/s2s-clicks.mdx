...
No API key is needed. The campaign is identified by `d` and `c`, and S2S clicks must be enabled for the advertiser's project.
...
|---|---|---|---|
| `d` | | Yes | Domain of your Linkrunner tracking link, for example `app.example.com`. |
| `c` | | Yes | Campaign code, the `c` value of your tracking link. Must match a campaign on that domain exactly. |
...
If both a name and its alias are sent, the name wins. `redirect`, `pid`, `af_siteid` and `af_lang` are ignored, so an AppsFlyer S2S template works once the host, `d` and `c` are changed.
...
|---|---|---|---|
| 200 | | Click recorded. | |
| 400 | `missing_d`, `missing_c`, `missing_tid` | A required parameter is missing or empty. | No |
...
| 400 | `unknown_campaign` | `c` does not match a campaign on that domain. | No |
...
```bash cURL
curl -G "https://s2s.linkrunner.io/v1/click" \
  --data-urlencode "d=app.example.com" \
...
const res = await fetch(`https://s2s.linkrunner.io/v1/click?${params}`);
...
res = requests.get("https://s2s.linkrunner.io/v1/click", params={
    "d": "app.example.com",
    "c": "OgWmhiSXhG",
    "tid": click_id,
    "gaid": advertising_id,
...
```
https://s2s.linkrunner.io/v1/click?d=app.example.com&c=OgWmhiSXhG&tid=a1b2c3d4e6&ip=49.36.10.20&ua=Mozilla%2F5.0%20(Linux%3B%20Android%2014)
```
