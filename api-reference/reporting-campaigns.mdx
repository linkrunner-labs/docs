...
| `cost_per_event`     | Return `cost_per_custom_events` / `cost_per_payment_events` for these names (spend divided by that event's count).      | `cost_per_event=Purchase,PURCHASE` |
| `unique_user_events` | Count distinct users for these events instead of occurrences, replacing that event's `count` with `unique_count`.       | `unique_user_events=Purchase`      |
...
Each parameter controls its own column family, so ask for each one you want. `events=Purchase` returns the Purchase count but no cost-per column; add `cost_per_event=Purchase` for that.
...
{
    "custom_events": {
        "Purchase": { "unique_count": "412", "amount": "0.00" },
...
`Purchase` reports distinct users because it was named in `unique_user_events`; `AddToCart` reports total occurrences. `payment_events` is empty because no revenue types were requested.
