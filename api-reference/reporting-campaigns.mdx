...
- **A cost-per column for every one of those events**, under `cost_per_custom_events` / `cost_per_payment_events`.
- **`active_users`** with a 7-day window.

<Note>
    Projects emitting more than 100 distinct event names get the first 100, ordered
    the same way [Dashboard → Events](https://dashboard.linkrunner.io/dashboard/events)
    lists them. Request anything beyond that explicitly with `events=`.
</Note>
...
| `cost_per_event`      | Narrow `cost_per_custom_events` / `cost_per_payment_events` to only these event names.                                             | `cost_per_event=Purchase,PURCHASE`   |
...
<Warning>
    `events`, `payment_events` and `cost_per_event` work as a group: send **any** of
    them and you take over the whole event column spec, so the other two narrow to
    exactly what you passed rather than defaulting to everything. Sending
    `events=Purchase` alone therefore returns no cost-per columns, so pass
    `cost_per_event=Purchase` alongside it if you want them.
</Warning>

<Note>
    **Counts are occurrences unless you ask otherwise.** `custom_events` and
    `payment_events` report total occurrence counts. To get distinct users for an
    event, name it in `unique_user_events`, which replaces that event's `count`
    with `unique_count`.

    This is a change if you called the API before 2026-09-11. Event counts used to
    follow whichever events a dashboard user had marked as unique-user in their
    saved column settings, so the same field could carry distinct users for one
    project and occurrences for another, with nothing in the response to tell you
    which. The API no longer reads those settings. If your numbers dropped for an
    event, it was previously being counted per user: add it to
    `unique_user_events` to get the old value back.
</Note>
