...
| event_id   | string | **Optional**. Unique event identifier (String). Recommended to use UUID format for uniqueness |
...
<Note>
    The `event_id` parameter should be a unique String identifier for each event. It's recommended to use a UUID (Universally Unique Identifier) format to ensure uniqueness. This helps track and deduplicate events on the server side.
</Note>
...
```javascript
// Using fetch API with crypto.randomUUID() (available in Node.js 14.17.0+ and modern browsers)
// For older environments, use a UUID library like 'uuid'
...
fetch("https://api.linkrunner.io/api/v1/capture-event", {
    ...
    body: JSON.stringify({
        ...
        user_id: "user_12345",
        event_id: uuidv4(), // Recommended: Use UUID for uniqueness
        // Alternative: event_id: crypto.randomUUID() // Available in Node.js 14.17.0+ and modern browsers
    }),
})
