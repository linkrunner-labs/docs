...
func trackEvent() async {
    do {
        try await LinkrunnerSDK.shared.trackEvent(
            ...
            eventId: UUID().uuidString // Optional: Unique event identifier (String). Recommended to use UUID for uniqueness
        )
...
<Note>
    The `eventId` parameter should be a unique String identifier for each event. It's recommended to use `UUID().uuidString` to generate a unique identifier. This helps track and deduplicate events on the server side.
</Note>
