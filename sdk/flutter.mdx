...
<Note>
    **Best practice:** set the Customer User ID as early as possible — the `user_id` is only attached to events tracked **after** it's set, and is not applied retroactively.
</Note>
