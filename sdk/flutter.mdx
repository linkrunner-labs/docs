...
```dart
try {
  await LinkRunner().setCustomerUserId("123"); // Your user ID
...
<Note>
    The identifier is stored securely on-device and persists across app restarts. Calling `setCustomerUserId` again will **not** overwrite an existing identifier. Only `signup()` / `setUserData()` update it.
</Note>
