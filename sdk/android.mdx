...
<Note>
    Available from **Android SDK v3.9.0**.
</Note>
...
private fun setCustomerUserId() {
    CoroutineScope(Dispatchers.IO).launch {
        val result = LinkRunner.getInstance().setCustomerUserId("123") // Your user ID
        result
...
<Note>
    The identifier is stored encrypted on-device and persists across app restarts. Calling `setCustomerUserId` again will **not** overwrite an existing identifier. Only `signup()` / `setUserData()` update it.
</Note>
