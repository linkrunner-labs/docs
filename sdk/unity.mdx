...
<Note>
The bridge code in this guide is reference code, so the shape is whatever your bridge serializes. The iOS example omits `adNetwork`, `type`, `installedAt`, and `storeClickAt`, and the Android example forwards the native object's `toString()` under a `raw` key. Extend the bridge serialization if you need the full `campaignData` field set consistently on both platforms.
</Note>
