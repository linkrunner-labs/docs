...
<AccordionGroup>
    ...
    <Accordion title="Get your Web SDK token">
        1. Open your project in the [Linkrunner dashboard](https://linkrunner.io/dashboard)
        2. Go to **Settings → Whitelist SDKs**
...
<Steps>
    ...
    <Step title="Add the checkout pixel">
        ...
        <Warning>
            **Customer email and phone are commented out by default.** Uncommenting them sends
            your shoppers' contact details to Linkrunner, where they are used to key audience
            exports to an identifiable person. Attribution, campaign reporting, and revenue all
            work without them.

            Only enable them if you actually need person-level exports, your privacy notice
            covers sharing contact details with Linkrunner, and your data processing agreement
            with us is in place.
        </Warning>
    ...
    <Step title="Verify it works">
        ...
        3. Open **Web Events** in your Linkrunner dashboard
...
<AccordionGroup>
    ...
    <Accordion title="Requires Web SDK v0.1.13 or later">
        Older versions did not store the last-touch traffic source, so the checkout pixel falls
        back to the visitor's first-touch source. Using the `cdn.linkrunner.io` URL in Step 1
...
| Checkout | `purchase` event with order value, currency, order ID, and item count. Customer email and phone are optional and off by default |
