...
<Info>
**For Expo SDK version 3.0.0 and higher:** All functionality is imported from `rn-linkrunner` (React Native SDK), not `expo-linkrunner`. The `expo-linkrunner` package is used solely as a config plugin for native configuration. The usage is identical to the React Native SDK.

For complete usage instructions, please refer to the [React Native Usage Guide](/sdk/react-native/usage).

For migration instructions from older versions, see the [Migration Guide](/sdk/expo/installation#migration-guide-for-version-3-0-0).
</Info>
