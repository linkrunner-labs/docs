...
1. **Prebuild your project** (if using a custom development client):

    ```bash
    npx expo prebuild
    ```

2. **Follow the React Native usage guide**: Since `expo-linkrunner` is a config plugin that prepares your project, the actual SDK usage is identical to React Native. Please refer to the [React Native Usage Guide](/sdk/react-native/usage) for:
