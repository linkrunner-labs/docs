...
func setCustomerUserId() async {
    await LinkrunnerSDK.shared.setCustomerUserId("123") // Your user ID
}
```

<Note>
    The identifier is stored in the Keychain and persists across app restarts. Calling `setCustomerUserId` again will **not** overwrite an existing identifier — only `signup()` / `setUserData()` update it.
</Note>
