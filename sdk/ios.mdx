...
<Steps>
  ...
  <Step title="Add the required linker flags">
    Google's framework needs these in **Build Settings** → **Other Linker Flags** on your app target:

    ```
    -ObjC -lc++
    ```

    CocoaPods adds them for you. Swift Package Manager does not.
  </Step>
...
`odm_available=false` with `odm_fetch_result=unavailable` means Google's SDK is not linked. Recheck step 1.
