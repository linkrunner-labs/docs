...
If you see "This CAPI token cannot access the dataset. Check its dataset permissions.", Linkrunner rejected the new configuration before saving. The dataset ID and token may not match, or the token may lack access to the dataset. This does not necessarily mean the token is invalid.
...
2. In that same dataset, open **Settings → Conversions API** and generate an access token. Paste the complete token into Linkrunner. If you see **Saved token**, click **Replace token** first.
3. If you cannot access the dataset or do not see **Generate access token**, ask the dataset owner or business admin to check your access and help generate the token. For a token generated through your own Meta app, have the admin check that the dataset/pixel is assigned to the system user that generated it. See [Meta's access token setup guide](https://developers.facebook.com/docs/marketing-api/conversions-api/get-started/#access-token). Do not grant broad permissions to unrelated assets as a workaround.
4. Click **Check access & save** again.
