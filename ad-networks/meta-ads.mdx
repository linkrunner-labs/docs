...
Linkrunner checks whether the token can access the dataset before saving. If the check fails, see [CAPI dataset access errors](#capi-dataset-access-errors).
...
If the modal says it could not verify dataset access or asks you to check dataset permissions, Linkrunner rejected the new configuration before saving. The dataset ID and token may not match, or the token may lack access. A valid Events Manager token can also be rejected because of a Linkrunner validation limitation. The error alone does not prove that you need a new token or broader permissions.
...
**Configuration saved** with **Dataset access checked** confirms access to the dataset, not event delivery. Continue with [event mapping](#2-set-up-event-mapping) and verify receipt in Meta Events Manager separately.
