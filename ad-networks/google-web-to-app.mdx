...
- **Google Web to App Ads**: everything above, plus permission to upload in-app conversions to your Google Ads conversion actions. Choose this for web-to-app so Google can optimize your web-to-app campaigns on real in-app events.
...
<Step title="Select Demand Gen" id="select-demand-gen">

On **Select a campaign type**, choose **Demand Gen**.

![Google Ads campaign type screen with Demand Gen selected](/images/google_ads/web-to-app/campaign-type-demand-gen.png)

<Note>
  **Why Demand Gen?** Performance Max campaigns do not support targeting iOS
  users only. Demand Gen supports OS-level device targeting, so you can serve
  the campaign to iOS devices only.
</Note>
...
<Step title="Set device targeting" id="set-device-targeting">

Attribution needs the ad click and the install to happen on the same device. Target only the devices that can install your app:

- **Mobile Phones**: check this. It is the right choice for almost every app.
- **Tablets**: check this only if your app is available on tablets and you want tablet installs. If not, a tablet click cannot become an attributable install.
- **Computers and TV screens**: leave unchecked. These devices cannot install your app, so their clicks spend budget without ever producing an attributable install.

In your campaign settings, expand the **Devices** section:

1. Select **Set specific targeting for devices**
2. Check **Mobile Phones** (and **Tablets** if your app supports them)
3. Under **Advanced targeting for devices**, click **Operating Systems** and select **iOS**

![Google Ads Demand Gen Devices section with Mobile Phones checked and iOS selected under Operating Systems](/images/google_ads/web-to-app/demand-gen-device-targeting-ios.png)

<Note>
  Run one campaign per platform. For your Android campaign, select **Android**
  instead of iOS.
</Note>
...
When you create your ad, set its **Final URL** to your Linkrunner domain:
...
![Demand Gen ad Final URL field with the Linkrunner domain filled in](/images/google_ads/web-to-app/demand-gen-ad-final-url.png)
...
After installs are attributed, Linkrunner can upload in-app conversions (such as purchases or sign-ups) back to Google Ads. Google uses them to optimize your Search and Demand Gen campaigns on real outcomes, not just installs.
