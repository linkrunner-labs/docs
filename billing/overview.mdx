...
Retargeting — re-engaging a lapsed user with a new ad — is billed exactly like any other install. There's no separate retargeting rate.

**A reattributed user is a billable install.** If a lapsed user returns through a paid or owned source and Linkrunner attributes that return within the [reattribution window](/features/remarketing), it counts as one attributed install at your normal per-install tier rate.
...
As always, we never bill the same install twice — a user counts once per attributed install, whether it's their first install or a reattribution.
