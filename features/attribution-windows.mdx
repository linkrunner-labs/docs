...
Each row is one ad network, and shows the window currently in effect along with where that number came from:

- **Default for this network**: the platform default for Google or Meta.
- **Default**: the 7 day platform default.
- No label: a value you set for this project.

Click the edit icon on a row, enter a number of days, and save. The change applies to that ad network on this project only. Use the reset icon to return a row to its default.
