---
title: "Retention"
description: "How Linkrunner calculates cumulative, classic, and rolling retention, explained with a simple example"
icon: "chart-line"
---

Retention tells you how many of your installs keep coming back to your app. Linkrunner reports it three ways: **cumulative**, **classic**, and **rolling**. The number shown simply as **Retention** in the dashboard (and the `retention` field in the API) is **cumulative retention**. This page explains all three and how the numbers are calculated.
...
| --- | --- |
| **Cumulative** (shown as **Retention**) | on **any day from Day 1 through Day N** |
...
**The Retention, Classic, and Rolling columns show different numbers.** This is expected. Retention (cumulative) counts installs that returned at any point in the first N days, classic counts only installs that returned on day N exactly, and rolling counts installs that returned on day N or later. Each answers a different question, so the numbers rarely match.
